...
function ScoreHistoryTable({ points }: { points: PluginScoreHistoryPoint[] }) {
  ...
  const visiblePoints = newestFirst.slice(0, 2);
  const remainingPoints = newestFirst.slice(2);
  ...
  return (
    <div>
        ...
        {remainingPoints.length ? (
          <details className="group overflow-hidden rounded-md border border-line bg-background">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3 text-sm transition hover:bg-surface-subtle focus-visible:bg-surface-subtle focus-visible:outline-none [&::-webkit-details-marker]:hidden">
              <span className="font-medium group-open:hidden">
                Show {remainingPoints.length.toLocaleString()} more
              </span>
              <span className="hidden font-medium group-open:inline">Show less</span>
              <ChevronDown
                size={17}
            ...
            <div className="space-y-3 border-t border-line p-3">
              {remainingPoints.map((point) => (
                <ScoreHistoryCard key={point.auditRunId} point={point} />
              ))}
            </div>
          </details>
        ) : null}
      ...
      <div className="hidden overflow-x-auto sm:block">
        ...
        {remainingPoints.length ? (
          <details className="group mt-3 overflow-hidden rounded-md border border-line bg-background">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3 text-sm transition hover:bg-surface-subtle focus-visible:bg-surface-subtle focus-visible:outline-none [&::-webkit-details-marker]:hidden">
              <span className="font-medium group-open:hidden">
                Show {remainingPoints.length.toLocaleString()} more
              </span>
            ...
            <div className="overflow-x-auto border-t border-line">
              <table className="min-w-[680px] w-full table-fixed border-collapse text-sm">
                ...
                <tbody>
                  {remainingPoints.map((point) => (
                    <ScoreHistoryRow key={point.auditRunId} point={point} />
                  ))}
                </tbody>
              </table>
            </div>
