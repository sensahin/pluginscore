...
export default async function CompareRoutePage({ params }: CompareRouteProps) {
  ...
  const entries = rawEntries
    ...
    .map((entry, index) => ({
      ...entry,
      color: chartColors[index % chartColors.length] ?? "var(--brand)",
    }));
...
function IssueCategoryList({ plugin }: { plugin: PluginDetail }) {
  ...
  const categories = groupFindings(plugin.topFindings ?? []).slice(0, 3);
...
function pluginDisplayName(plugin: PluginDetail) {
...
function groupFindings(findings: FindingCodeCount[]) {
  const groups = new Map<string, { family: string; total: number }>();

  for (const finding of findings) {
    const existing = groups.get(finding.family) ?? {
